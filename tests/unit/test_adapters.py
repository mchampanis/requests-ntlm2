...
import requests_ntlm3.adapters
import requests_ntlm3.connection


class TestHttpProxyAdapter(unittest.TestCase):
    def test_init(self):
        adapter = requests_ntlm3.adapters.HttpProxyAdapter()
        self.assertIsInstance(adapter, requests_ntlm3.adapters.HttpProxyAdapter)
    ...
    @mock.patch("requests_ntlm3.adapters.HttpProxyAdapter._add_host_header")
...
class TestHttpNtlmAdapter(unittest.TestCase):
    @mock.patch("requests_ntlm3.adapters.HttpNtlmAdapter._teardown")
    @mock.patch("requests_ntlm3.adapters.HttpNtlmAdapter._setup")
    def test_init(self, mock_setup, mock_teardown):
        adapter = requests_ntlm3.adapters.HttpNtlmAdapter("username", "password")
        self.assertIsInstance(adapter, requests_ntlm3.adapters.HttpNtlmAdapter)
        self.assertIsInstance(adapter, requests_ntlm3.adapters.HttpProxyAdapter)
        self.assertIsInstance(adapter, requests.adapters.HTTPAdapter)
    ...
    @mock.patch("requests_ntlm3.connection.HTTPSConnection.set_ntlm_auth_credentials")
    def test__setup(self, mock_set_ntlm_auth_credentials):
        ...
        self.assertTrue(http_conn_cls, requests_ntlm3.connection.HTTPConnection)
        self.assertTrue(https_conn_cls, requests_ntlm3.connection.HTTPSConnection)
        adapter.close()

    @mock.patch("requests_ntlm3.connection.HTTPSConnection.clear_ntlm_auth_credentials")
    @mock.patch("requests_ntlm3.connection.HTTPSConnection.set_ntlm_auth_credentials")
    def test_close(self, set_ntlm_auth_credentials, clear_ntlm_auth_credentials):
        from requests.packages.urllib3.poolmanager import pool_classes_by_scheme
        adapter = requests_ntlm3.adapters.HttpNtlmAdapter("username2", "password")
